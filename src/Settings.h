...
static const FieldInfo gGlobalPrefsFields[] = {
    ...
    {offsetof(GlobalPrefs, cacheDir), SettingType::String, 0},
...
static const StructInfo gGlobalPrefsInfo = {
    ...
    "mePageViewMode\0CacheDir\0FilePicker\0ReloadModifiedDocuments\0RememberOpenedFiles\0RememberStatePerDocument\0Rest"
    "oreSession\0ReuseInstance\0ShowMenubar\0ShowMenubarWithTabs\0ShowTips\0CustomColors\0ShowToolbar\0Toolbar\0Toolbar"
    "Position\0SearchUIFloating\0ShowFavorites\0SortFavoritesByName\0ShowToc\0SidebarOnRight\0ShowLinks\0ClickEdgeToTur"
    "nPage\0DisableLinks\0RememberViewOffsetOnPageTurn\0ShowDocumentFocusIndicator\0ShowAnnotationNotification\0ShowToc"
    "PageNumbers\0ShowStartPage\0SidebarDx\0Scrollbars\0ScrollbarInSinglePage\0SmoothScroll\0ScrollLineAmount\0PaddingA"
    "fterLastPage\0IgnoreDestinationZoom\0CitationHoverDelay\0ReadAloudVoiceId\0ReadAloudSpeed\0FastScrollOverScrollbar"
    "\0PreventSleepInFullscreen\0TabWidth\0Theme\0LastLightTheme\0LastDarkTheme\0DocumentColorsFollowTheme\0TocDy\0Tool"
    "barCustomLayout\0ToolbarShowReadAloud\0ToolbarSize\0TreeFontName\0TreeFontSize\0UIFontSize\0DisableAntiAlias\0Engi"
    "neeringDrawingEnhance\0DisableAutoLinks\0UseSysColors\0UseTabs\0SelectionToolbar\0TabsMru\0CtrlTabSimple\0ZoomLeve"
    "ls\0ZoomIncrement\0\0FixedPageUI\0\0EBookUI\0\0ComicBookUI\0\0ImageUI\0\0ChmUI\0\0MarkdownUI\0\0HtmlUI\0\0ClaudeCo"
    "de\0\0GrokBuild\0\0CodexBuild\0\0AntiGravity\0\0AIChatSidebarDx\0\0TranslateToLang\0TranslateFromLang\0TranslateEn"
    "gine\0\0Annotations\0\0ExternalViewers\0\0ForwardSearch\0\0PrinterDefaults\0\0Fullscreen\0\0SelectionHandlers\0\0S"
    "hortcuts\0\0Themes\0\0TabGroups\0\0CustomScreenDPI\0\0\0DefaultPasswords\0UiLanguage\0VersionToSkip\0WindowState\0"
    "WindowPos\0SearchUIWindowPos\0FileStates\0SessionData\0ReopenOnce\0TimeOfLastUpdateCheck\0OpenCountWeek\0PropWinPo"
    "s\0CheckForUpdates\0\0",
    ...
    "first\0valid values: thumbnails, list\0directory used to store cached document thumbnails. Empty uses the "
    "sumatrapdfcache directory in SumatraPDF's application data directory\0valid values: (empty), os, sumatrapdf\0if "
    "true, a document will be reloaded automatically whenever it's changed (currently doesn't work for documents shown "
    "in the ebook UI)\0if true, remember which documents were opened and their display settings\0if true, store "
    "display settings for each document separately (i.e. everything after UseDefaultState in FileStates)\0if true and "
    "SessionData isn't empty, that session will be restored at startup\0if true, open documents in the already running "
    "SumatraPDF instead of starting a new one\0if true, show the menu bar (F9 toggles it; the choice is remembered "
    "across sessions)\0if true, show the menu bar when using tabs (useTabs = true)\0if true, show tips on the home "
    ...
    "right-to-left UI languages already put it on the right)\0if true, draw a blue border around links in the "
    "document\0if true, a click (not a drag) on the left fifth of the page area goes to the previous page and a click "
    "on the right fifth goes to the next page (reversed in manga / right-to-left mode). Links, annotations and "
    "presentation-mode clicks are unchanged\0if true, document links are ignored so you can select and read (useful "
    "for drawings with many links); if false, clicking a link follows it\0if true, next/previous page keeps the same "
    "view position on the page instead of jumping to the top (useful when zoomed in on similarly sized pages)\0if "
    "true, draw a focus ring around the document when it has keyboard focus (Tab to the page area)\0if true, show a "
    "tip when hovering an annotation (e.g. \"Highlight annotation. Ctrl+click to edit.\")\0if true, show page numbers "
    ...
    "as well\0if true, smooth mouse-wheel scrolling (exponential chase of the target; continuous wheel input stays "
    "fluid)\0distance, in screen pixels at 96 DPI, scrolled by an arrow-key press or one mouse-wheel line; values "
    "below 1 use 16\0if true, continuous view has extra scroll room after the last page so you can scroll the end of "
    "the document to the top of the window\0if true, going to a destination (clicking a bookmark or a link inside the "
    ...
    "Destination actions'\0how long an internal-document link has to be hovered, in milliseconds, before a popup "
    "rendering the destination region (citation entry, figure, footnote) appears. -1 (the default) disables the popup; "
    "set a positive value like 300 to enable it\0voice id for Read Aloud text-to-speech; empty or unset means system "
